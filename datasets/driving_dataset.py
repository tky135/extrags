    ...
    def project_lidar_pts_on_images(self, delete_out_of_view_points=True):
        ...
        for cam in self.pixel_source.camera_data.values():
            ...
            for frame_idx in tqdm(
                ...
                on_road_mask_in_valid = road_mask[cam_points[valid_mask, 1].long(), cam_points[valid_mask, 0].long()] == 1
                
                valid_mask_clone = valid_mask.clone()
                valid_mask_clone[valid_mask] = off_road_mask_in_valid
                
                valide_mask_clone_on_road = valid_mask.clone()
                valide_mask_clone_on_road[valid_mask] = on_road_mask_in_valid
                ...
                road_indices = torch.arange(
                    self.lidar_source.num_points, device=self.device
                )[lidar_infos["lidar_mask"]][valid_mask_on_road]
                
                self.lidar_source.visible_masks[visible_indices] = True
                self.lidar_source.road_masks[road_indices] = True
