...
class MagicDrive:
    def __init__(self, sd_path, checkpoint_path, version="trainval", nuscenes_root='/train-syncdata/kaiyuan.tan/nuscenes-download/nuscenes'):
        ...
        self.lpips_loss = lpips.LPIPS(net='vgg').to(torch.device('cuda:0')) # input scaled to [-1, 1]
    ...
    def get_loss(self, pred_rgb, sample_token, timestep, shift_x, step, method, inpainting_mask=None, **kwargs):
        ...
        info['pixel_values'] = pred_rgb.detach().clone() # no grad 
        ...
        visualize_class_map(batch['bev_map_with_aux'][0], os.path.join(prefix_current_time, f"segmentation_shifted_{sample_token}_{step}.png"))
        # hack bboxes TODO
        ...
        if method == 'sds':
            ...
            with torch.no_grad():
                ...
                for i in timesteps:
                    ...
                    output = self.noise_scheduler.step(model_pred[0], i, noisy_latents[0], inpainting_mask_vae=inpainting_mask_vae[0].half(), gt_vae_latents=sds_img_vae[0], stochastic=True)
        ...
        #     return total_loss, {'target_image': image.detach(), 'target_latent': noisy_latents_sds.detach()}
            ...
            with torch.no_grad():
                ...
                noisy_latents = rearrange(noisy_latents, 'b c ... -> (b c) ...')
                image = self.vae.decode(noisy_latents).sample
                image = rearrange(image, '(b c) ... -> b c ...', b=bs).detach()
                # image = (image / 2 + 0.5).clamp(0, 1)
            ...
            image = normalize_image(image.float())
            pred_rgb = normalize_image(pred_rgb)


            image = image.to(pred_rgb.device)
            loss_mse = F.mse_loss(pred_rgb, image.float(), reduction='mean')

            loss_l1 = F.l1_loss(pred_rgb, image.float(), reduction='mean')

            # loss_latent_mse = F.mse_loss(sds_img_vae, noisy_latents_sds.detach(), reduction='sum')
            loss_latent_mse = 0.0 # latent detached for now

            loss_lpips = self.lpips_loss(pred_rgb.squeeze(0), image.float().squeeze(0)).mean()
...
if __name__ == "__main__":
    ...
    for pt_path, scene, it in iterable:
        ...
        # save_dict['pred_rgb'] =/ torch.zeros_like(save_dict['pred_rgb'])
        # save_dict['pred_rgb'] = F.interpolate(save_dict['pred_rgb'].squeeze(0), size=(424, 800), mode='bilinear', align_corners=False).unsqueeze(0)
        # save_dict['inpainting_mask'] = F.interpolate(save_dict['inpainting_mask'], size=(424, 800), mode='bilinear', align_corners=False)
        if method == 'direct':
            mgd.get_loss(pred_rgb=save_dict['pred_rgb'], sample_token=save_dict['sample_token'], timestep=999, shift_x=save_dict['shift_x'], step=it, method='direct', inpainting_mask=torch.ones_like(save_dict['inpainting_mask']), scene_idx=0)
            ...
            for i in range(1, 201):
                ...
                # inpainting_mask = (save_dict['inpainting_mask'] > 0).float()
                loss, ret_dict = mgd.get_loss(pred_rgb=image_params_masked, sample_token=save_dict['sample_token'], timestep=None, shift_x=save_dict['shift_x'] , step=i, method=method, inpainting_mask=torch.ones_like(inpainting_mask), **kwargs)
